...
def checkout(request, service_id):
    """Display checkout page and handle booking submission."""
    service = get_object_or_404(Service, id=service_id)
    ...
    if request.method == 'GET':
        ...
        intent = stripe.PaymentIntent.create(
            amount=int(service.price * 100),  # Convert to pence
...
def success(request, order_id):
    """Render the success page and send booking confirmation email."""
    ...
    if request.method == "POST":
        ...
        if 'checkout_services' in request.session:
            ...
            if service_id in checkout_services:
                ...
                messages.success(
                    request,
                    f"Service '{service.name}' has been removed from your "
                    "checkout."
                )
        else:
